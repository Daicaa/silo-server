	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			node := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
				switch r.Method {
				case http.MethodGet:
					if r.Header.Get(playback.TranscodeProxyRequestHeader) != "1" {
						t.Error("segment request omitted proxy marker")
					}
					seenRange <- r.Header.Get("Range")
					w.Header().Set(playback.TranscodeSegmentGenerationHeader, generation)
					http.ServeContent(w, r, "seg_00007.ts", time.Time{}, strings.NewReader(body))
				case http.MethodPost:
					if r.Header.Get(playback.TranscodeSegmentGenerationHeader) != generation {
						t.Errorf("ack generation = %q, want %q", r.Header.Get(playback.TranscodeSegmentGenerationHeader), generation)
					}
			...
			if got := rr.Header().Get(playback.TranscodeSegmentGenerationHeader); got != "" {
...
func TestProxyToTranscodeNodeDoesNotAcknowledgeFailedDownstreamWrite(t *testing.T) {
	...
	node := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		w.Header().Set(playback.TranscodeSegmentGenerationHeader, "21")
		http.ServeContent(w, r, "seg_00007.ts", time.Time{}, strings.NewReader(body))
	}))
	defer node.Close()
